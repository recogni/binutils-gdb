...
target_alias=riscv32
target_vendor=unknown
target_os=none
target=riscv32-unknown-none

program_transform_name = s&^&riscv32-&
...
SUBDIRS =  intl libiberty opcodes bfd readline zlib libdecnumber libctf binutils gas ld sim gdb gprof etc gnulib
...
CC_FOR_TARGET=$(STAGE_CC_WRAPPER) riscv32-cc

# If GCC_FOR_TARGET is not overriden on the command line, then this
# variable is passed down to the gcc Makefile, where it is used to
# build libgcc2.a.  We define it here so that it can itself be
# overridden on the command line.
GCC_FOR_TARGET=$(STAGE_CC_WRAPPER) riscv32-gcc
CXX_FOR_TARGET=$(STAGE_CC_WRAPPER) riscv32-c++
RAW_CXX_FOR_TARGET=$(STAGE_CC_WRAPPER) riscv32-c++
GFORTRAN_FOR_TARGET=$(STAGE_CC_WRAPPER) riscv32-gfortran
GOC_FOR_TARGET=$(STAGE_CC_WRAPPER) riscv32-gccgo
DLLTOOL_FOR_TARGET=$$r/$(HOST_SUBDIR)/binutils/dlltool
LD_FOR_TARGET=$$r/$(HOST_SUBDIR)/ld/ld-new

LIPO_FOR_TARGET=riscv32-lipo
